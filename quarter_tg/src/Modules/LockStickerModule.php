...
class LockStickerModule
{
    ...
    public function handle(array $update, array $args, TelegramApi $api, string $command): void
    {
        ...
        $this->lockManager->setLock($chatId, 'stickers', true);

        $response = $lang === 'fa'
            ? "🔒 قفل استیکر فعال شد. کاربران غیرمدیر نمی‌توانند استیکر ارسال کنند."
            : "🔒 Sticker lock enabled. Non-admin users cannot send stickers.";
