...
class UnlockPhotoModule
{
    ...
    public function handle(array $update, array $args, TelegramApi $api, string $command): void
    {
        ...
        $this->lockManager->setLock($chatId, 'photos', false);

        $response = $lang === 'fa'
            ? "🔓 قفل عکس غیرفعال شد. همه کاربران می‌توانند عکس ارسال کنند."
            : "🔓 Photo lock disabled. All users can send photos.";
