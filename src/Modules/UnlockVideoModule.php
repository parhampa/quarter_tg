...
class UnlockVideoModule
{
    ...
    public function handle(array $update, array $args, TelegramApi $api, string $command): void
    {
        ...
        $this->lockManager->setLock($chatId, 'videos', false);

        $response = $lang === 'fa'
            ? "🔓 قفل فیلم غیرفعال شد. همه کاربران می‌توانند فیلم ارسال کنند."
            : "🔓 Video lock disabled. All users can send videos.";
